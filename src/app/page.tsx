  ...
  return (
    <div id="home-page" className="min-h-screen">
      ...
      <section id="hero" aria-label="メインビジュアル" className="relative h-screen -mt-16 pt-16">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-white z-10 pt-16 hero-section" style={{backgroundImage: 'url(/images/hero.jpeg)', backgroundSize: 'cover', backgroundPosition: 'center'}}>
          <motion.div id="hero-content" className="space-y-8 -translate-y-12">
            <motion.h1 
      ...
      <section id="features" aria-label="SOLAIZの特徴" className="py-20 px-4 bg-[var(--background)]">
        <div className="container mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            <AnimatedSection delay={0.1} id="feature-location">
            ...
            <AnimatedSection delay={0.2} id="feature-comfort">
              <div className="text-center">
                <div className="bg-[var(--primary)] text-[var(--primary-foreground)] w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                  <FiHome size={24} />
                </div>
            ...
            <AnimatedSection delay={0.3} id="feature-experience">
              <div className="text-center">
                <div className="bg-[var(--primary)] text-[var(--primary-foreground)] w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                  <FiCalendar size={24} />
                </div>
      ...
      <section id="rooms" aria-label="お部屋のご案内" className="py-20 px-4 bg-[var(--muted)]">
        <div className="container mx-auto">
          <AnimatedSection id="rooms-header">
          ...
          <div id="rooms-container" className="flex overflow-x-auto pb-6 gap-6 scrollbar-hide">
            {featuredRooms.map((room, index) => (
              <AnimatedSection key={room.id} delay={0.1 * index} id={`room-card-${room.id}`}>
                <RoomCard {...room} id={`room-${room.id}`} />
              </AnimatedSection>
            ))}
          </div>
          
          <div id="rooms-cta" className="text-center mt-8">
            <Link 
              href="/rooms" 
              className="inline-block border-2 border-[var(--primary)] text-[var(--primary)] px-6 py-2 rounded-full hover:bg-[var(--primary)] hover:text-[var(--primary-foreground)] transition-colors"
      ...
      <section id="access" aria-label="アクセス情報" className="py-20 px-4 bg-[var(--background)]">
        <div className="container mx-auto">
          <AnimatedSection id="access-header">
            <h2 className="text-3xl md:text-4xl font-bold text-center mb-4 text-[var(--heading)]">
              アクセス
            </h2>
            <p className="text-center text-[var(--muted-foreground)] mb-12 max-w-3xl mx-auto">
          ...
          <AnimatedSection delay={0.1} id="access-map-container">
            <div className="bg-[var(--card)] rounded-lg shadow-md overflow-hidden max-w-4xl mx-auto">
              <div id="access-map" className="h-96 relative">
                <iframe 
              ...
              <div id="access-info" className="p-6">
      ...
      <section id="contact" aria-label="お問い合わせ" className="py-20 px-4 bg-[var(--muted)]">
        <div className="container mx-auto text-center">
          <AnimatedSection id="contact-content">
            <h2 className="text-3xl md:text-4xl font-bold mb-6 text-[var(--heading)]">
              あなただけの島時間を、SOLAIZで
            </h2>
            <p className="mb-8 max-w-2xl mx-auto text-lg text-[var(--foreground)]">
              石垣島の豊かな自然と海、そしてスタッフの魅力ある日常を感じながら、忘れられない旅の思い出を。
            </p>
            <Link 
              href="/contact" 
              id="contact-cta-btn"
              className="bg-[var(--primary)] text-[var(--primary-foreground)] px-8 py-3 rounded-full font-medium hover:bg-[var(--primary-dark)] transition-colors inline-block"
            >
              お問い合わせ
            </Link>
          </AnimatedSection>
